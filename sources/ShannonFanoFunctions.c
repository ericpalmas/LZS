...
void compress(char *compFileName, unsigned char *codedBuffer, int codedBufferCounter,FILE *compressed) {
...
unsigned char *writeDecompBuffer(Node *root, unsigned char *bufferIn, int bufferSize) {
    ...
    unsigned int initialeBuffSize = 512;
    unsigned char *bufferOut = malloc(initialeBuffSize * sizeof(unsigned char));
    ...
    while (indexBuffIn < bufferSize) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if (root->leftChild == NULL) {
                if (indexBuffOut == initialeBuffSize - 1) {
                    initialeBuffSize <<= 1;
                    bufferOut = realloc(bufferOut, initialeBuffSize * sizeof(unsigned char));
                }
    ...
    decompBufferSize = indexBuffOut;
    return bufferOut;
}

unsigned int getDecompBufferSize() {
    return decompBufferSize;
}
